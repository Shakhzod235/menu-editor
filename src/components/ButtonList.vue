  ...
  <div class="flex w-full gap-2">
    <div class="flex w-full gap-2 flex-col">
      <div v-for="(row, rowIndex) in buttonStore.buttons" class="check flex justify-between w-full gap-2 flex-wrap">
        ...
        <button v-if="buttonStore.isAddColumnBtnVisible(rowIndex)" @click="displayStore.toggleElement('addColumn'), buttonStore.toggleAddColumnBtn(rowIndex), body.classList.toggle('overflow-hidden')" class="add-column-btn border border-dashed border-gray-300 hover:border-gray-500 hover:text-gray-500 px-3 rounded-lg focus:outline-none focus-visible:border-indigo-600 transition">
